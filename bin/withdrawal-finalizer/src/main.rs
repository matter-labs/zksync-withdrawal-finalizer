...
use sqlx::{postgres::{PgConnectOptions,PgPoolOptions}, ConnectOptions, PgConnection};
...
async fn main() -> Result<()> {
    ...
    let pgpool = PgPoolOptions::new().acquire_timeout(Duration::from_secs(2)).connect_with(options).await?;
